...
eap_client_init(SupplicantRef supp, EAPType type)
{
    ...
    *((bool *)&supp->eap.plugin_data.log_enabled) = ((eapolclient_log_flags() & kLogFlagDisableInnerDetails) == 0);
	
    *((bool *)&supp->eap.plugin_data.system_mode) = (EAPOLSocketGetMode(supp->sock) == kEAPOLControlModeSystem);
    supp->last_status =
...
S_check_for_updated_credentials(SupplicantRef supp)
{
	...
	EAPLOG(LOG_ERR, " ####### DEBUG ####### - S_check_for_updated_credentials");
...
S_set_credentials(SupplicantRef supp)
{
	...
	EAPLOG(LOG_ERR, " ####### DEBUG ####### - S_set_credentials");
...
#if ! TARGET_OS_EMBEDDED
	EAPLOG(LOG_ERR, " ####### DEBUG ####### - System_mode = %d", system_mode);
    ...
    if (system_mode
		...
		if (name != NULL && password != NULL) {
			...
			EAPLOG(LOG_NOTICE,
				   "System Mode using OD account '%s' and password '%s'", name, password);
		}
		else {
								...
								bool * should_stop)
{
	...
	EAPLOG(LOG_ERR, " ####### DEBUG ####### - Supplicant_update_configuration");
