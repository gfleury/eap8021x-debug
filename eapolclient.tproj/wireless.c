...
wireless_is_wpa_enterprise(wireless_t wref)
{
	...
	EAPLOG(LOG_ERR, "######## DEBUG ####### - wireless_is_wpa_enterprise");
	...
	/*
	size = CFDictionaryGetCount(info);
	CFTypeRef *keysTypeRef = (CFTypeRef *) malloc( size * sizeof(CFTypeRef) );
	CFDictionaryGetKeysAndValues(info, (const void **) keysTypeRef, NULL);
	const void **keys = (const void **) keysTypeRef;
	
	for (int x=0; x < size; x++) {
		EAPLOG(LOG_ERR, "####### DUMP ###### - %d %d %.3s", x, keys[x], keys[x]);
		if (isA_CFString(keys[x])) {
			char *salve = MYCFStringCopyUTF8String(keys[x]);
			EAPLOG(LOG_ERR, "######## DEBUG ####### - EH UM CFSTRING %s", salve);
			free(salve);
		}
		if (isA_CFNumber(keys[x])) {
			EAPLOG(LOG_ERR, "######## DEBUG ####### - EH UM CFNUMBER");
		}
		upper_cf = CFDictionaryGetValue(info, keys[x]);
		if (upper_cf) {
			if (CFNumberGetValue(upper_cf, kCFNumberSInt32Type, &upper) != FALSE) {
				switch (upper) {
					case APPLE80211_AUTHTYPE_WPA:
					case APPLE80211_AUTHTYPE_WPA2:
						EAPLOG(LOG_ERR, "##### DEBUG ##### - Numero %d", upper);
...
CFDictionaryRef wireless_copy_scan_record (char  *c, SCDynamicStoreRef ref){
	return NULL;
}
...
boolean_t wireless_reassociate (wireless_t wref, CFDictionaryRef dict) {
	EAPLOG(LOG_ERR, "######## DEBUG ####### - FAIL - wireless_reassociate");
